...
def showGrid(grid, nbv=None, predicted_nbv=None):
    ...
    def set_axes_equal(ax):
        # Ensure the aspect ratio is 1:1:1 for the 3D plot
    ...
    # plot the NBV
    # the view sphere was placed at 0.4 m from the origin, the voxel map has an approximate size of 0.25
    scale = 32 / 2
    ...
    if nbv is not None:
        ...
        ax.quiver(position[0], position[1], position[2], direction[0], direction[1], direction[2],
                  length=9.0, normalize=True, color='g')
    ...
    if predicted_nbv is not None:
        ...
        direction = center - position
        ax.quiver(position[0], position[1], position[2], direction[0], direction[1], direction[2],
                  length=9.0, normalize=True, color='r')
