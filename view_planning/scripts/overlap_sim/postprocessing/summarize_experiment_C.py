...
# -*- coding: utf-8 -*-
"""
Postprocessing for Experiment C (repulsion gain sweep)

Reads CSVs named `stability_C_*.csv` from ../logs and produces:
  - spacing_vs_step.png          : step vs min & mean spacing for each k_rep
  - lambda_vs_step_clipped.png   : step vs lambda_max_real (clipped, transient removed)
  - summary_vs_krep.png          : final spacing and final lambda_max_real vs k_rep
  - lambda_vs_spacing.png        : final lambda_max_real vs final mean_spacing (one dot per k_rep)
...
OUT_DIR = os.path.join(".", "figures_experiment_C")
...
def collect_by_krep(pattern="stability_C_*.csv"):
    ...
    data_by_k : dict[float, dict]
        ...
        Typically each k_rep will have exactly one CSV.
    """
...
def main():
    ...
    # ------------------------------------------------------------------
    # Plot 3: final spacing & lambda vs k_rep
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Plot 4: λ_max vs mean spacing (one point per k_rep)
    # ------------------------------------------------------------------
