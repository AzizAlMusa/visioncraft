...
# trail_length = 40  # Removed - trails will grow indefinitely
...
dot1, = ax.plot([], [], [], 'o', color='#ffff00', markersize=10)
dot2, = ax.plot([], [], [], 'o', color='#00ffff', markersize=10)

trail_line1, = ax.plot([], [], [], color='#ffff00', linewidth=2, alpha=0.8)
trail_points1, = ax.plot([], [], [], 'o', color='#ffff00', markersize=2, alpha=0.6)

trail_line2, = ax.plot([], [], [], color='#00ffff', linewidth=2, alpha=0.8)
trail_points2, = ax.plot([], [], [], 'o', color='#00ffff', markersize=2, alpha=0.6)
...
# Add some retro text annotations
fig.text(0.02, 0.95, 'MULTI-AGENT PHYSICS SIMULATION', 
         color='#00ff41', fontsize=12, fontproperties=bold_font)
fig.text(0.02, 0.92, 'STATUS: ACTIVE', 
         color='#00ff41', fontsize=10, fontproperties=custom_font)
...
def create_attractor_arrows():
    ...
    # No central star - just the arrows pointing to the attractor point
...
def animate(frame):
    ...
    # No trail length limiting - trails grow forever!
    ...
    animate.overlap_text_obj = fig.text(0.02, 0.80, overlap_text, 
                                      color=overlap_color, fontsize=9, 
                                      fontproperties=custom_font)
...
anim = animation.FuncAnimation(fig, animate, frames=2000, 
                             interval=30, blit=False, repeat=True)  # Reduced from 50ms to 30ms

# Remove grid lines for clean look
# ax.grid(False)  # Turn off grid completely

plt.tight_layout()

# To save as GIF (optional):
# anim.save('retro_multi_agent_sphere.gif', writer='pillow', fps=20)

plt.show()

print("Multi-agent physics simulation created!")
print("Yellow dot and cyan dot are attracted to the red star but repel each other.")
print("Their Gaussian bumps represent repulsive force fields.")
