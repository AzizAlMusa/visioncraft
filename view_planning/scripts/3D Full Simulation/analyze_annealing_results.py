#!/usr/bin/env python3
"""
Post-processing script for 3D Simulated Annealing (SA) results.

Supports both layouts:
1) ./sa/seed_0, seed_1, ...
2) ./sa/<model>/seed_0, seed_1, ...
...
def load_seed_data(base_dir, strategy):
    ...
    for seed_dir in seed_dirs:
        ...
        pattern = f"{strategy}_result.npz"  # expects e.g., sa_result.npz
...
def create_time_plots(sa_data, save_dir, title_suffix=""):
    """Create plots vs. time for SA data and save to save_dir."""
    if not sa_data:
    ...
    fig.suptitle(f'Simulation Results Over Time (SA){title_suffix}', fontsize=16)

    label = 'SA'
    color = 'orange'
    max_t = max(float(np.max(d['time'])) for d in sa_data)
    common_time = np.linspace(0.0, max_t, 500)

    cov_interp = np.array([np.interp(common_time, d['time'], d['coverage'])   for d in sa_data])
    red_interp = np.array([np.interp(common_time, d['time'], d['redundancy']) for d in sa_data])
    aff_interp = np.array([np.interp(common_time, d['time'], d['affinity'])   for d in sa_data])
    ...
    axes[0, 0].plot(common_time, cov_mean, label=f'{label} (n={len(sa_data)})', color=color)
    axes[0, 0].fill_between(common_time, cov_mean - cov_std, cov_mean + cov_std, alpha=0.3, color=color)

    axes[0, 1].plot(common_time, red_mean, label=f'{label} (n={len(sa_data)})', color=color)
    axes[0, 1].fill_between(common_time, red_mean - red_std, red_mean + red_std, alpha=0.3, color=color)

    axes[1, 0].plot(common_time, aff_mean, label=f'{label} (n={len(sa_data)})', color=color)
...
def process_one_model(model_dir, strategy="sa", title_suffix=""):
...
def main():
    ...
    parser.add_argument('--results_dir', type=str, default='./sa',
                        help="Path to results root. Supports ./sa/seed_* or ./sa/<model>/seed_*.")
    ...
    for model_dir in model_dirs:
        ...
        process_one_model(model_dir, strategy="sa", title_suffix=f" — {name}")
