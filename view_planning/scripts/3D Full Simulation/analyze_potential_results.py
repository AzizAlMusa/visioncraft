#!/usr/bin/env python3
"""
Post-processing script for 3D Potential Field simulation results.

Supports both layouts:
1) ./potential_field/seed_0, seed_1, ...
2) ./potential_field/<model>/seed_0, seed_1, ...
Each model's outputs are saved inside that model's directory.
"""
...
def load_seed_data(base_dir, strategy):
    ...
    for seed_dir in seed_dirs:
        ...
        pattern = f"{strategy}_result.npz"  # expects potential_field_result.npz
...
def create_time_plots(potential_data, save_dir, title_suffix=""):
    """Create plots vs. time for Potential Field data."""
    ...
    fig.suptitle(f'Simulation Results Over Time (Potential Field){title_suffix}', fontsize=16)

    label = 'Potential Field'
    color = 'green'
    max_t = max(float(np.max(d['time'])) for d in potential_data)
    common_time = np.linspace(0.0, max_t, 500)

    cov_interp = np.array([np.interp(common_time, d['time'], d['coverage'])   for d in potential_data])
    red_interp = np.array([np.interp(common_time, d['time'], d['redundancy']) for d in potential_data])
    aff_interp = np.array([np.interp(common_time, d['time'], d['affinity'])   for d in potential_data])
    ...
    axes[0, 0].plot(common_time, cov_mean, label=f'{label} (n={len(potential_data)})', color=color)
    axes[0, 0].fill_between(common_time, cov_mean - cov_std, cov_mean + cov_std, alpha=0.3, color=color)

    axes[0, 1].plot(common_time, red_mean, label=f'{label} (n={len(potential_data)})', color=color)
    axes[0, 1].fill_between(common_time, red_mean - red_std, red_mean + red_std, alpha=0.3, color=color)

    axes[1, 0].plot(common_time, aff_mean, label=f'{label} (n={len(potential_data)})', color=color)
    ...
    axes[0, 0].legend()
    plt.tight_layout()
    path = os.path.join(save_dir, 'potential_over_time.png')
...
def list_model_dirs(results_dir):
    """
    If results_dir directly contains seed_* => old layout: treat results_dir as one 'model'.
    Else => new layout: return subdirectories that contain seed_*.
    """
...
def process_one_model(model_dir, strategy="potential_field", title_suffix=""):
...
def main():
    ...
    parser.add_argument('--results_dir', type=str, default='./potential_field',
                        help="Path to results root. Supports ./potential_field/seed_* or ./potential_field/<model>/seed_*.")
    ...
    for model_dir in model_dirs:
        ...
        process_one_model(model_dir, strategy="potential_field", title_suffix=f" — {name}")
