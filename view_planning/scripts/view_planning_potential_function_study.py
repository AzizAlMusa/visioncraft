...
class AngularViewPlanningField:
    ...
    def compute_potential(self, viewpoints):
        """
        Compute potential for each object point based on angular distance only.
        True 1D manifold formulation: linear potential in angular space.
        """
        # Extract angular positions of viewpoints (in radians)
        viewpoint_vectors = viewpoints[:, :2] - circle_center
        viewpoint_angles = np.arctan2(viewpoint_vectors[:, 1], viewpoint_vectors[:, 0])
        viewpoint_angles = np.mod(viewpoint_angles, 2*np.pi)

        self.obj_potentials.fill(0.0)  # Reset

        for i, theta_obj in enumerate(self.obj_manifold_projections):
            if self.obj_visibility[i] >= 1.0:
                continue  # Already covered fully

            # Sum linear angular distances to all viewpoints
            linear_sum = 0.0
            for theta_v in viewpoint_angles:
                delta = theta_obj - theta_v
                delta = np.arctan2(np.sin(delta), np.cos(delta))  # Wrap
                linear_sum += np.abs(delta)

            self.obj_potentials[i] = (1.0 - self.obj_visibility[i]) * linear_sum
    ...
    def compute_angular_forces(self, angular_positions, alpha=1.0, k_rep=0.2):
        """
        Compute angular forces using true 1D formulation:
        Linear potential, angular-only metric.
        """
        ...
        for j in range(num_viewpoints):
            ...
            for i, theta_i in enumerate(self.obj_manifold_projections):
                if self.obj_potentials[i] <= 0.0:
                    continue  # No influence

                delta = theta_i - theta_j
                delta = np.arctan2(np.sin(delta), np.cos(delta))  # Wrap to [-π, π]
                sign = np.sign(delta)

                # Linear force model (constant in angular space)
                force += self.obj_potentials[i] * sign

            angular_forces[j] = alpha * force

        # --- Repulsive Force Between Viewpoints (Gaussian) ---
        if k_rep > 0 and num_viewpoints > 1:
            ...
            for i in range(num_viewpoints):
                for j in range(num_viewpoints):
                    if i == j:
                        continue
                    delta = angular_positions[j] - angular_positions[i]
                    delta = np.arctan2(np.sin(delta), np.cos(delta))
                    repulsion = -np.sign(delta) * k_rep * np.exp(-(delta**2) / (2*sigma**2))
                    angular_forces[i] += repulsion
