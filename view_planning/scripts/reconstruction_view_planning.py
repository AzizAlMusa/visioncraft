...
class AngularViewPlanningField:
    def __init__(self, grid_size, num_particles, fov_angle, fov_radius):
        ...
        self.viewpoint_scans = None  # Will store scan data for each viewpoint
        self.viewpoint_scan_quality = None  # Store visibility/quality of each scan point
    ...
    def initialize_viewpoints(self, num_viewpoints, radius, center):
        ...
        # Initialize viewpoint scans
        self.viewpoint_scans = [[] for _ in range(num_viewpoints)]
    ...
    def perform_ray_casting(self, viewpoints, num_rays=50):
        """
        Perform ray casting from each viewpoint to simulate range sensor readings.
        Returns scan points for each viewpoint with Gaussian noise based on visibility.
        """
        # Reset viewpoint scans
        self.viewpoint_scans = [[] for _ in range(viewpoints.shape[0])]
        # Store visibility values for each scan point to use in visualization
        self.viewpoint_scan_quality = [[] for _ in range(viewpoints.shape[0])]
        
        # For each viewpoint
        for i, viewpoint in enumerate(viewpoints):
            position = viewpoint[:2]
            direction = viewpoint[2:]
            
            # Calculate the base angle (angle of the direction vector)
            base_angle = np.arctan2(direction[1], direction[0])
            
            # Calculate the range of angles for the field of view
            half_fov_rad = np.radians(self.fov_angle / 2)
            start_angle = base_angle - half_fov_rad
            end_angle = base_angle + half_fov_rad
            
            # Generate rays within the field of view
            ...
            for angle in ray_angles:
                ...
                # Find closest intersection with object points
                closest_point = None
                min_distance = float('inf')
                
                for obj_point in self.object_points:
                    # Vector from viewpoint to object point
                    to_obj = obj_point - position
                    distance = np.linalg.norm(to_obj)
                    
                    # Project onto ray direction to check if it's in front
                    proj = np.dot(to_obj, ray_dir)
                    
                    # Only consider points in front of the ray and within FOV radius
                    if proj > 0 and distance < self.fov_radius:
                        # Calculate perpendicular distance to the ray
                        perp_dist = np.linalg.norm(to_obj - proj * ray_dir)
                        
                        # If close enough to the ray and closer than previous points
                        if perp_dist < 0.5 and distance < min_distance:
                            min_distance = distance
                            closest_point = obj_point
                
                # If found an intersection, calculate visibility and add noise
                if closest_point is not None:
                    # Calculate angle relative to the viewpoint's direction
                    to_obj = closest_point - position
                    to_obj_normalized = to_obj / (np.linalg.norm(to_obj) + epsilon)
                    cos_angle = np.dot(to_obj_normalized, direction)
                    angle_diff = np.arccos(np.clip(cos_angle, -1.0, 1.0))
                    
                    # Calculate visibility using same method as in can_see_point
                    distance = np.linalg.norm(to_obj)
                    ...
                    angle_threshold = np.radians(self.fov_angle / 2)
                    angle_sharpness = 20.0
                    angle_factor = 1.0 / (1.0 + np.exp(angle_sharpness * (angle_diff/angle_threshold - 0.95)))
                    
                    # Combined visibility
                    visibility = distance_factor * angle_factor
                    
                    # Calculate noise based on visibility
                    # More noise for lower visibility (at edges of FOV or at distance limit)
                    noise_level = max(0.01, 1.0 - visibility) * 2.0  # Scale noise level
                    
                    # Apply Gaussian noise proportional to distance and inverse of visibility
                    noise_x = np.random.normal(0, noise_level)
                    noise_y = np.random.normal(0, noise_level)
                    
                    # Create noisy point
                    noisy_point = closest_point + np.array([noise_x, noise_y])
                    
                    # Save the noisy point and its quality
                    self.viewpoint_scans[i].append(noisy_point)
                    self.viewpoint_scan_quality[i].append(visibility)
        
        return self.viewpoint_scans, self.viewpoint_scan_quality
...
viewpoint_scans, viewpoint_scan_quality = field.perform_ray_casting(viewpoints, num_rays=50)
...
# Reconstruction plot (replacing Energy plot)
ax_reconstruction = fig.add_subplot(gs[0, 3])
ax_reconstruction.set_xlim(0, grid_size)
ax_reconstruction.set_ylim(0, grid_size)
ax_reconstruction.set_aspect("equal")
ax_reconstruction.set_title("Viewpoint Reconstruction", fontsize=10)

# Generate distinct colors for each viewpoint
distinct_colors = plt.cm.tab10(np.linspace(0, 1, num_particles))
...
# Polygon for reconstruction (reference outline)
reconstruction_polygon = plt.Polygon(object_vertices, fill=False, alpha=0.3, color='black', linestyle='--')
ax_reconstruction.add_patch(reconstruction_polygon)
...
# Add small viewpoint markers in the reconstruction panel
reconstruction_viewpoint_scatter = ax_reconstruction.scatter(
    viewpoints[:, 0], viewpoints[:, 1],
    c=distinct_colors,
    s=30, 
    zorder=4,
    marker='^'
...
# Scan point visualization in reconstruction plot
scan_scatters = []
for i in range(num_particles):
    if viewpoint_scans[i]:
        # Convert lists to numpy arrays
        points = np.array(viewpoint_scans[i])
        quality = np.array(field.viewpoint_scan_quality[i])
        
        # Use quality values to set the size of points
        # Higher quality = slightly larger
        sizes = 10 + quality * 20  # Size range: 10-30 based on quality
        
        # Create scatter with fixed alpha and variable size
        scatter = ax_reconstruction.scatter(
            points[:, 0], points[:, 1],
            color=distinct_colors[i],
            s=sizes,
            alpha=0.8,  # Use a fixed alpha initially
            zorder=3
        )
        scan_scatters.append(scatter)
    else:
        # Add empty scatter if no points yet
        scatter = ax_reconstruction.scatter(
            [], [],
            color=distinct_colors[i],
            s=15,
...
# Add legend for reconstruction plot
legend_elements = []
for i in range(num_particles):
    legend_elements.append(plt.Line2D([0], [0], marker='o', color='w', 
                                      markerfacecolor=distinct_colors[i], markersize=8,
                                      label=f'Viewpoint {i+1}'))
    
ax_reconstruction.legend(handles=legend_elements, loc='upper right', fontsize=8)
...
def update(frame):
    global viewpoints, angular_positions, m_angular, v_angular, t_angular, fov_patches, scan_scatters
    ...
    # Perform ray casting for reconstruction visualization
    viewpoint_scans, viewpoint_scan_quality = field.perform_ray_casting(viewpoints, num_rays=50)
    ...
    reconstruction_viewpoint_scatter.set_offsets(viewpoints[:, :2])
    ...
    # 5. Update reconstruction visualization
    for i, scatter in enumerate(scan_scatters):
        if viewpoint_scans[i]:
            points = np.array(viewpoint_scans[i])
            quality = np.array(field.viewpoint_scan_quality[i])
            
            # Update offsets (positions)
            scatter.set_offsets(points)
            
            # Update sizes based on quality
            sizes = 10 + quality * 20  # Size range: 10-30
            scatter.set_sizes(sizes)
            
            # Instead of directly setting alpha array, adjust color with transparency
            rgba_colors = np.zeros((len(points), 4))
            # Set RGB from the distinct color
            rgba_colors[:, 0:3] = mcolors.to_rgb(distinct_colors[i])
            # Set alpha channel based on quality
            rgba_colors[:, 3] = 0.3 + quality * 0.7
            
            scatter.set_facecolor(rgba_colors)
        else:
    ...
    return [object_scatter, viewpoint_scatter, manifold_projected_scatter, manifold_object_scatter, 
            manifold_viewpoint_scatter, coverage_line, stats_text, reconstruction_viewpoint_scatter] + scan_scatters
...
plt.tight_layout()
plt.show()
# ani.save('view_planning_with_reconstruction.mp4', writer='ffmpeg', fps=120, dpi=100)