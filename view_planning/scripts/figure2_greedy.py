        ...
        for dx in (-grid_size,0,grid_size):
            for dy in (-grid_size,0,grid_size):
                ax.add_patch(plt.Circle((p[0]+dx, p[1]+dy), fov_radius,
                                        ...
                                        alpha=0.2, lw=1))
    ax.scatter(pts[:,0], pts[:,1], s=30, c='deepskyblue',
...
if args.animate and len(snapshots) >= 2:
    ...
    plot_frame(ax[0], viewpoints[:1], 1-snapshots[0].reshape(grid_size,grid_size),
               title="Initial")
    plot_frame(ax[1], viewpoints,    1-snapshots[-1].reshape(grid_size,grid_size),
               title="Final")
